...
export const createOrder = async ({ cartId }: { cartId: string }): Promise<OrderWithRelations | null> => {
    const res = await api.post('/orders', { cartId });
    return res.data.success ? res.data.order : null;
}

export const getOrderById = async ({ orderId }: { orderId: string }): Promise<OrderWithRelations | null> => {
