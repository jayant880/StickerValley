...
const reviewController = {
    addReview: asyncHandler(async (req: Request, res: Response) => {
        ...
        const result = await db
            .insert(reviews)
            .values({ userId: user.id, stickerId: sticker.id, rating, comment })
            .returning();
    ...
    getReviewsByStickerId: asyncHandler(async (req: Request, res: Response) => {
        const sticker = req.sticker;
        const reviewsByStickerId = await db.query.reviews.findMany({
        ...
        return res.status(200).json({
            ...
            data: reviewsByStickerId,
        });
    }),

    getReviewByUserId: asyncHandler(async (req: Request, res: Response) => {
        const { userId } = req.params;
        const reviewsByUser = await db.query.reviews.findMany({
