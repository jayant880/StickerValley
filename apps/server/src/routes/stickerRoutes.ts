...
router.post("/", requireUser, requireShopByUserId, validate(createStickerSchema), stickerController.addSticker);
router.get("/:id", requireSticker, stickerController.getStickerById);
router.put("/:id", requireUser, requireShopByUserId, requireSticker, requireStickerOwnership, validate(updateStickerSchema), stickerController.updateSticker);
